...
- ✅ Detector rail (entity detection, meta 0x08 power, scheduled tick re-poll)
...
- ✅ **Jukebox** (84) — disc insert/eject on right-click, stop music + drop disc on break
- ✅ **Flower Pot** (140) — plant placement on right-click, drop contained plant on break
- ❌ Trip wires
...
- **BiomeGenRegistry** — 430-line header, 40 base biomes + 20 mutated variants with heights/temperature/rainfall/surface blocks
- **NoiseGen** — 391 lines, Perlin/Simplex noise generators
- **GenLayer** — 835-line header, full biome generation pipeline (17 layer types: Island→FuzzyZoom→AddIsland→AddSnow→Edge→Biome→Hills→RareBiome→Shore→Smooth→RiverMix→VoronoiZoom)
- **WorldChunkManager** — biome query interface (1:4 scale for height blending, 1:1 for per-block biome assignment)
- ✅ **Biome-aware terrain** — density computation uses biome rootHeight/variation, surface replacement uses biome topBlock/fillerBlock
- ✅ **Biome IDs in chunks** — per-block biome array populated from WorldChunkManager
- ✅ **Snow/ice placement** — cold biomes get snow layers on solid blocks, ice on water
- ✅ **Biome-specific trees** — oak/birch/spruce/jungle/acacia/dark oak selected by biome type with correct tree density
- ✅ **Biome-specific decorations** — flowers, tallgrass, cacti, dead bush, lily pads, sugar cane density varies by biome
- ~~❌ Only flat generator actually works; no terrain/caves/ores~~ ✅ OverworldGenerator with caves + ores + biome-aware terrain
...
- **Entity packets** — ~~S0C SpawnPlayer~~, S0E SpawnObject, ~~S0F SpawnMob~~, ~~S10 SpawnPainting~~, ~~S12 EntityVelocity~~, S14 Entity, ~~S15 EntityRelMove~~, ~~S16 EntityLook~~, ~~S17 EntityLookRelMove~~, ~~S18 EntityTeleport~~, ~~S19 EntityHeadLook~~, ~~S1A EntityStatus~~, S1B AttachEntity, ~~S1C EntityMetadata~~, ~~S1D EntityEffect~~, ~~S1E RemoveEntityEffect~~
...
- ~~**Player data persistence**~~ — ✅ save/load position, rotation, health, food, XP, **inventory, ender chest, game mode, selected slot** via NBT to world/playerdata/<uuid>.dat
...
- ~~**Nether/End portal transit**~~ — ✅ Nether: standing in portal 80 ticks (4s) triggers dimension transfer, Overworld↔Nether coord scaling (÷8/×8), portal search/creation in dest world. End: instant transfer on block 119, Overworld→End 5×5 obsidian platform at (100,49,0), End→Overworld respawn at world spawn, End→End credits via S2B reason 4. S07 Respawn, chunk streaming, potion re-send, 300-tick cooldown
...
- ~~**Caves**~~ — ✅ MapGenCaves worm carving integrated; ✅ MapGenRavine ravine carving integrated (1/50 chance per chunk, worm-based with 3:1 vertical aspect, lava below y=10, water occlusion)
- ~~**Ores**~~ — ✅ All 11 vanilla ores (coal, iron, gold, diamond, redstone, lapis, emerald, dirt, gravel)
- **Structures** — ✅ Dungeons (cobblestone/mossy rooms with mob spawner); ✅ Mineshafts (corridors with fence+plank supports, rails, cobwebs, torches, cave spider spawners); ✅ Scattered Features (desert pyramids with TNT trap + 4 chests, jungle pyramids with tripwire/piston traps + 2 chests, swamp huts with cauldron + witch spawner); ✅ Strongholds (11 piece types + corridor fallback, recursive generation, End portal room with silverfish spawner, libraries with bookshelves + chests, stonebrick/mossy/cracked/silverfish block selector, ring-based placement around spawn); ✅ Villages (12 piece types — Well center, Path gravel roads, Torch lamp posts, WoodHut, Church, House1 library, House2 blacksmith with chest+furnaces+lava, House3 two-roof, House4Garden terrace, Hall butcher, Field1 4-crop farm, Field2 2-crop farm; grid-based spawning maxDist=32/minDist=8/salt=10387312; weighted piece selection; desert variant sandstone substitution; biome-aware plains/desert/savanna); ✅ **Structure chest loot** (ChestRegistrar callback fills pendingChests on chunk load with weighted random loot tables — dungeon/mineshaft/desert_pyramid/jungle_pyramid/stronghold_corridor/stronghold_crossing/stronghold_library/blacksmith/nether_fortress/bonus_chest)
...
235. **Animal feeding** — ✅ Done (Java EntityAnimal.interact: per-mob breeding items — cow/mooshroom/sheep→wheat 296, pig→carrot 391, chicken→seeds 295/361/362; sets inLoveTicks=600, S1A EntityStatus byte 18 love hearts, consumes in survival; rejects if already in love or on cooldown)
236. **Animal breeding** — ✅ Done (Java EntityAIMate: in-love same-type mobs within 8 blocks pair up, 60-tick proximity countdown → spawn baby at midpoint, both parents get 6000-tick cooldown, 7 heart particles burst, 1-7 XP to nearest player; heart particles every 10 ticks while in love)
237. **Bonemeal growth** — ✅ Done (Java ItemDye.func_150919_a → IGrowable.fertilize: dye 351:15 on wheat/carrots/potatoes 59/141/142 → +2-5 stages capped at 7; nether wart 115 → +1 capped at 3; melon/pumpkin stems 104/105 → +2-5 capped at 7; cocoa 127 → +1 stage; saplings 6 → set stage bit; grass 2 → spawn 2-4 tall grass/flowers nearby; consumes in survival, effect 2005 green particles)
238. **Wolf taming/AI** — ✅ Done (Java EntityWolf: bone taming 1/3 chance, sit/stand toggle, meat healing, collar dyeing, breeding, DW 16/17/18/19/20 metadata, follow owner with teleport >12 blocks, attack target defense, ambient/hurt/death sounds, wolf speed 0.30f, health 8→20 on tame)
239. **Ocelot taming/AI** — ✅ Done (Java EntityOcelot: fish taming 1/3 chance, catSkinType 1-3 random, sit/stand toggle, breeding with fish, DW 16/18 metadata, follow owner with teleport >12 blocks, ambient sounds mob.cat.meow/purr/purreow when tamed, hurt/death mob.cat.hitt, health 10.0f, speed 0.30f)
240. **Creeper-ocelot avoidance** — ✅ Done (Java EntityCreeper.EntityAIAvoidEntity(EntityOcelot.class, 6.0f): creepers flee from ocelots/cats within 6 blocks, cancel fuse, 1.5× flee speed)
241. **Arrow-vs-mob collision** — ✅ Done (Java EntityArrow.onUpdate() entity scan: arrows now collide with all mob types, not just players; damage=ceil(speed×baseDamage)+critical bonus, 20 mob-type hurt sounds, Enderman arrow immunity with bounce+teleport, Punch knockback (0.6×kb/horizDist), Flame setFire, random.bowhit pitch=1.2/(rand×0.2+0.9); on death: 28 mob death sounds, S13 DestroyEntities, full drop tables for 20+ mob types with Looting from shooter's held item, XP orbs via spawnXPOrbs; isDead guard prevents double-hit with player collision)
...
257. **Detector rail** — ✅ Done (Java BlockRailDetector parity: entity detection via player AABB intersection in PacketHandler, meta bit 0x08 powered state, click sound on activate/deactivate, redstoneNotifyNeighbors + block below, 20-tick scheduled re-poll for deactivation, power source in calculateSignal/isIndirectlyPowered/lamp/TNT/note block/repeater checks, placement redstone notify)
258. **Mineshaft generation** — ✅ Done (Java MapGenMineshaft+StructureMineshaftPieces parity: cross-chunk scanning 4-radius, JavaRNG deterministic seeding, 4 piece types — Room cavity with dirt floor, Corridor 3-wide with fence+plank support beams every 5 blocks + rails 70% + cobwebs + torches 5%, Cross 4-way intersection with optional multi-floor, Stairs 8-block descent; spider corridors 1/23 with 60% cobweb + cave spider mob spawner; 0.4% spawn chance with distance-from-spawn check)
259. **Scattered feature generation** — ✅ Done (Java MapGenScatteredFeature+ComponentScatteredFeaturePieces parity: grid-based spawning maxDist=32/minDist=8/salt=14357, biome-dependent structure selection; DesertPyramid 21×15×21 sandstone with stepped towers+wool patterns+underground TNT trap+pressure plate+4 chests; JunglePyramid 12×10×15 randomized cobblestone/mossy 30%+2 tripwire→dispenser arrow traps+3-lever sticky piston puzzle+hidden chest; SwampHut 7×5×9 spruce on oak stilts+cauldron+crafting table+witch spawner; ChestInfo/pendingChests integration; LootTables.h MUSIC_DISC_13 forward-ref fix)
260. **Stronghold generation** — ✅ Done (Java MapGenStronghold+StructureStrongholdPieces parity: 1274-line header-only MapGenStronghold.h; 11 piece types — Stairs spiral descent, Crossing 4-way with random exits, Straight corridor with side branches, LeftTurn/RightTurn, Prison with iron bars+doors, ChestCorridor with loot, RoomCrossing 5 variants (empty/pillar+torch/water pool/chest/cobblestone pillars), StairsStraight 6-step descent, Library small/large with bookshelves+torches+chests, PortalRoom with End portal frames (10% eye-of-ender chance)+lava pool+silverfish spawner; Corridor fallback filler; Stones block selector randomizes stonebrick/mossy(30%)/cracked(20%)/silverfish(5%); PieceWeight system with weighted random selection+instance limits; recursive generation depth=50 maxPieces=75; ring-based placement 128 strongholds in concentric rings around spawn; integrated into OverworldGenerator Step 6.7)
261. **Village generation** — ✅ Done (Java MapGenVillage+StructureVillagePieces parity: 1334-line header-only MapGenVillage.h; 12 piece types — Well 6×11×6 cobblestone center with water+fence posts, Path gravel roads with branching, Torch 3×4×2 fence lamp posts with wool+torches, WoodHut 4×6×5 cobblestone+planks with tall/short variant, Church 5×12×9 cobblestone tower with ladder+glass panes, House1 9×9×6 library with bookshelves+crafting table, House2 10×6×7 blacksmith with furnaces+lava+iron bars+chest, House3 9×7×12 two-roof with log pillars+glass panes, House4Garden 5×6×5 with optional terrace fence+ladder, Hall 9×7×11 butcher with fenced courtyard+table+double stone slabs, Field1 13×4×9 four-crop farm with water channels+log borders, Field2 7×4×9 two-crop farm; grid-based spawning maxDist=32 minDist=8 salt=10387312; PieceWeight weighted random selection with instance limits; recursive path-first expansion from well center; desert biome variant substitutes sandstone for cobblestone/planks/logs; biome check plains(1)/desert(2)/savanna(35); integrated into OverworldGenerator Step 6.8)
262. **Ravine generation integration** — ✅ Done (existing 281-line MapGenRavine.h wired into OverworldGenerator Step 6.1 after caves; 17×17 source-chunk scanning loop applies BlockMod vector to blocks array; 1/50 chance per source chunk, worm carving with 3:1 vertical aspect ratio, sin-shaped width profile, 256-entry randomized width table, water occlusion skip, lava fill below y=10, biome top block exposure)
263. **Nether Fortress generation** — ✅ Done (Java MapGenNetherBridge+StructureNetherBridgePieces parity: 1132-line header-only MapGenNetherFortress.h; 14 piece types — Straight 5×10×19 bridge corridor, Crossing3 19×10×19 large 4-way intersection, Crossing 7×9×7, Stairs 7×11×7, Throne 7×8×9 with Blaze spawner, Entrance 13×14×13 with lava, Corridor5 5×7×5 basic, Crossing2 5×7×5 small 3-way, Corridor2 5×7×5 with chest, Corridor 5×7×5 with chest, Corridor3 5×14×10 stairway descent, Corridor4 9×7×9 T-junction, NetherStalkRoom 13×14×13 soul sand + nether wart farm, End 5×10×8 dead-end cap; PieceWeight weighted random selection with instance limits; recursive generation from Crossing3 start piece; JavaRNG deterministic seeding 16-chunk grid; block metadata for nether brick stairs/fences; pendingSpawners/pendingChests propagation to Chunk; wired into NetherGenerator Step 4 after caves; ChunkSection yBase fix + isTerrainPopulated)
264. **Biome-aware terrain generation** — ✅ Done (Java GenLayer.initializeAllBiomeGenerators+WorldChunkManager parity: 835-line GenLayer.h with 17 layer types in full pipeline, WorldChunkManager biome query interface at 1:4 and 1:1 resolution, BiomeGenRegistry 40+20 biomes; OverworldGenerator integration — density computation uses biome rootHeight/variation for height blending, surface replacement uses biome topBlock/fillerBlock, per-block biome IDs stored in chunk biome array, snow/ice placement in cold biomes, biome-specific tree selection (oak/birch/spruce/jungle/acacia/dark oak) with correct density, biome-specific decoration rates for flowers/tallgrass/cacti/dead bush/lily pads/sugar cane)
265. **Structure chest loot generation** — ✅ Done (Java WeightedRandomChestContent.generateChestContents parity: ChestRegistrar callback mirrors SpawnerRegistrar pattern, processes pendingChests on chunk load; LootTables.h generateChestContents with weighted random item selection, stack size ranges, max stack splitting; 10 loot tables — dungeon 13 items, mineshaft 16, desert_pyramid 14, jungle_pyramid 8, stronghold_corridor/crossing/library, blacksmith 15, nether_fortress 11, bonus_chest 7; Item::getItemById max stack lookup; seeded RNG per chest position)
266. **Village villager spawning** — ✅ Done (Java StructureVillagePieces$Village.spawnVillagers parity: VillagerInfo struct + pendingVillagers in Chunk.h; spawnVillagers helper in MapGenVillage.h with coordBaseMode-aware positioning; 8 building types spawn villagers — WoodHut farmer, Church priest, House1 librarian, House2 blacksmith, House3 farmer, House4Garden farmer, Hall butcher/farmer; VillagerRegistrar callback in World.h/World.cpp mirrors SpawnerRegistrar pattern; MinecraftServer wires registrar calling summonMob(120) + sets profession via SpawnedMob.villagerProfession + broadcasts DataWatcher DW 16 int; movement speed 0.5f for entity type 120)
267. **Fishing rod mechanics** — ✅ Done (Java EntityFishHook parity: SpawnedFishHook struct with 3-phase catch cycle — Phase 1 initial wait 100-900 ticks, Phase 2 fish approach 20-80 ticks with bubble/wake particles, Phase 3 bite window 10-30 ticks with splash sound + bobber dip; spawnFishHook with Gaussian inaccuracy + S0E SpawnObject type 90; tickFishHooks with water detection, buoyancy physics, ground collision + 1200-tick despawn, angler validation (rod held + 1024 distance check); retractFishHook with weighted loot tables — junk 10% (11 items weight 83), treasure 5% (6 items weight 6), fish 85% (cod 60/salmon 25/clownfish 2/pufferfish 13); Luck of the Sea enchant 61 adjusts junk/treasure chances, Lure enchant 62 reduces wait time by level×100 ticks; XP orb 1-6 on catch; item 346 handler in PacketHandler casts or retracts with durability damage 1=fish/2=ground; rain speedMod doubles catch rate)
268. **Horse riding movement** — ✅ Done (Java EntityHorse.moveEntityWithHeading parity: rider-controlled yaw/pitch, strafe*0.5/forward input with 0.25× backward penalty, rider walk speed 0.22477891f, jump via C0B action 6 with horseJumpStrength attribute, gravity 0.08-0.04 Nether, gallop sound every 220 ticks, mob.horse.gallop/land sounds, S12 EntityVelocity on jump, S18 EntityTeleport broadcast; vehicle dismount C0B action 5 resets riderEntityId + S1B AttachEntity detach)
269. **Horse inventory GUI** — ✅ Done (Java ContainerHorseInventory parity: S2D OpenWindow type 11 with entity ID, S30 WindowItems saddle/armor/chest/player slots; handleClickWindow type 11 — slot 0 saddle (item 329 only), slot 1 armor (417/418/419 normal horse only), slots 2-16 chest (chested donkey/mule), player inv mapping; DW 16/DW 22 metadata broadcast on saddle/armor change; horseChestInventory[15] persistent storage in SpawnedMob; closeOpenWindow resets horseEntityId_/horseSlotCount_; friend class PlayHandler for MinecraftServer private access)
270. **Villager trading** — ✅ Done (Java EntityVillager.interact+MerchantRecipe+MerchantRecipeList parity: MerchantRecipe struct in standalone header MerchantRecipe.h with buyItemId1/2, sellItemId, toolUses/maxTradeUses, isDisabled(); generateVillagerTrades for 5 professions — farmer 0 wheat/wool/chicken/fish/bread/melon/apple/cookie/shears/arrow/flint, librarian 1 paper/book/bookshelf/glass/compass/clock/enchanted book, priest 2 ender eye/exp bottle/redstone/glowstone, blacksmith 3 coal/iron/gold/diamond/25 tool+armor trades, butcher 4 coal/pork/beef/saddle/leather armor/cooked meat; probability-based generation from villagersSellingList+blacksmithSellingList ranges; villagerTradesGenerated lazy init on first interact; openVillagerTrading S34 plugin message with trade list serialization; updateMerchantOutput recipe matching with S2F SetSlot; ClickWindow handler for merchant slots 0-2 with trade execution, toolUses increment, input consumption; handleMerchantSlot2Click craft output with stack merging; villagerRecipes_ per-handler state; villagerEntityId_ tracking; mob.villager.haggle/yes sounds)
271. **Nether portal transit** — ✅ Done (Java Teleporter+ServerConfigurationManager.transferPlayerToDimension parity: tickPortal 80-tick standing detection + 300-tick cooldown, creative 1-tick, transferDimension with Overworld↔Nether coordinate scaling ÷8/×8 X/Z, 128-block radius portal search in destination world, portal creation 4×5 obsidian frame with portal blocks, Y clamp for Nether ceiling, S07 Respawn packet with dimension/difficulty/gamemode/leveltype, chunk unload+stream around destination, S08 PlayerPosAndLook, health/food/XP sync, inventory re-send, potion effect re-send via S1D, onPlayerJoined re-broadcast, savePlayerData with new dimension; getWorldForDimension out-of-line in MinecraftServer.cpp)
272. **End portal transit** — ✅ Done (Java BlockEndPortal.onEntityCollidedWithBlock+EntityPlayerMP.travelToDimension parity: tickPortal detects End portal block ID 119 for instant transfer (no 80-tick timer), transferDimension(conn, isEndPortal) with bool parameter; Overworld→End creates 5×5 obsidian platform at (100,49,0) per Teleporter.placeInPortal() for dimension 1, 3-block-tall air clearance; End→End sends S2B ChangeGameState reason 4 credits/end poem then transfers to Overworld; End→Overworld respawns at world spawn with Y-scan for valid ground; Nether portals disabled in End dimension; level type always "default" per Java parity; 300-tick cooldown)
273. **Ender Dragon boss** — ✅ Done (Java EntityDragon parity: tickDragon ~500 lines; 64-entry ring buffer movement interpolation, player/random target acquisition, flight physics with steering/speed modulation, Ender Crystal healing search every 10 ticks + heal 1HP/10 ticks, block destruction via head/body AABBs excluding obsidian/endstone/bedrock + mobGriefing check, wing knockback collideWithEntities, head melee 10 damage attackEntitiesInList, wing flap mob.enderdragon.wings sound, periodic mob.enderdragon.growl; handlePlayerAttack: head=full damage body=damage/4+1, only player/explosion sources, mob.enderdragon.hit/mob.enderdragon.end sounds; 200-tick death sequence: ticks 150-200 XP orbs 1000/batch every 5 ticks, tick 200 final 2000 XP + createEnderPortal at y=64 bedrock ring + end portal blocks + dragon egg + torches; SpawnedMob dragon fields: ringBuffer[64][3], deathTicks, healingCrystalId, animTime, motionX/Y/Z)
274. **Wither boss** — ✅ Done (Java EntityWither parity: tickWither ~200 lines; 220-tick spawn invulnerability with 10HP/10tick heal + mobSpell particles + effect 1015 sound + spawn explosion; 3-head targeting center 40 blocks + side 30 blocks; skull attacks center every 40 ticks 8 damage + Wither II 10s, side heads every 10-20 ticks; charged shots after 15 charges create explosion; armored mode at health≤150 immune to arrows; 3×4×3 block destruction on damage with mobGriefing check; 1HP/20tick regen; floating movement with 0.6× Y-damping; mob.wither.idle/hurt/death sounds; handlePlayerAttack invulnerability + armored checks + block break timer; death drops 50 XP + Nether Star item 399; SpawnedMob wither fields: witherInvulTime, witherWatchedTargets[3], sideHeadYaw/Pitch[2], sideAttackTimer/ChargeCounter[2], witherBlockBreakTimer, witherMotionX/Y/Z, witherTicksExisted)
275. **Golem building detection** — ✅ Done (Java BlockPumpkin.onBlockAdded parity: pumpkin/jack-o-lantern placement triggers golem pattern check; Snow Golem: 2 snow blocks 80 below pumpkin → clear 3 blocks + summonMob(97); Iron Golem: T-shape of 4 iron blocks 42 below pumpkin (body vertical + X/Z arms) → clear 5 blocks + summonMob(99); both types auto-marked isPassive=true in summonMob for despawn immunity)
276. **Snow Golem AI** — ✅ Done (Java EntitySnowman.onLivingUpdate+attackEntityWithRangedAttack parity: tickSnowGolem ~180 lines; water damage 1/tick when wet/rained on, heat damage 1/tick on hot ground (sand/clay/netherrack) with fire visual, snow trail 4 corner positions ±0.25 placing snow_layer 78 on solid blocks in cold areas, ranged snowball attack on nearest hostile mob within 10 blocks via spawnThrowable with Java setThrowableHeading speed=1.6 inaccuracy=12.0 + random.bow sound, 20-tick attack cooldown; death drops rand()%16 snowballs; snowGolemAttackCooldown field in SpawnedMob; integrated into tickMobs() with fall-through to passive wander AI)
277. **Wither building detection** — ✅ Done (Java BlockSkull.makeWither parity: skull block 144 placement stores skullType in skulls_ map via packBlockPos key + skullsMutex_; wither skeleton skull type 1 triggers pattern check — Z-axis and X-axis alignment: 3 wither skulls at y + T-shape of 4 soul sand 88 at y-1/y-2; pattern match clears 7 structure blocks via setBlockInWorld, spawns Wither summonMob(64), 120 snowballpoof particles via broadcastParticle; skull drop on break uses stored skullType with consistent packBlockPos + mutex protection)
278. **Painting entity** — ✅ Done (Java EntityPainting parity: SpawnedPainting struct with entityId/blockXYZ/hangingDirection/artTitle/sizeXY/isDead; S10 SpawnPainting packet with VarInt entityId + String artTitle + Int blockX/Y/Z + Int direction; spawnPainting creates entity + broadcasts S10 to all players; removePainting marks isDead + S13 DestroyEntities + drops item 321 in survival; case 321 placement logic — wall-face-only check, facingToDirection mapping, 26 EnumArt entries iterated with bounding box validation against solid backing blocks, random candidate selection; attack handler instant-kill on hit; player join broadcasts existing paintings; nextPaintingEntityId_ counter starting at 1200000)
279. **Item frame entity** — ✅ Done (Java EntityItemFrame parity: SpawnedItemFrame struct with entityId/blockXYZ/hangingDirection/displayedItemId/Damage/Count/rotation/isDead; S0E SpawnObject type 71 with hangingDirection as data field; spawnItemFrame creates entity + broadcasts to all; removeItemFrame marks isDead + S13 DestroyEntities + drops item 389 + displayed item in survival; interactItemFrame — empty frame: place held item + consume in survival, has item: rotate 0-7 + broadcast S1C metadata; case 389 placement logic — wall-face-only, solid backing block, air target, no duplicate frame check; attack handler two-hit Java parity — first hit drops displayed item + clears frame, second hit destroys empty frame; sendEntityMetadataItemFrame S1C with DataWatcher index 2 ItemStack + index 3 byte rotation; player join sync sends S0E + metadata for all existing frames; nextItemFrameEntityId_ starting at 1300000)
280. **Ender Crystal entity** — ✅ Done (Java EntityEnderCrystal parity: SpawnedEnderCrystal struct with entityId/posX/Y/Z/isDead; spawnEnderCrystal creates entity + S0E SpawnObject type 51 broadcast; removeEnderCrystal marks isDead + S13 DestroyEntities + createExplosion power=6.0; attack routing in handleUseEntity action=1 calls removeEnderCrystal; deferred spawning via pendingEnderCrystals in Chunk.h populated from EndGenerator.generateSpikes obsidian pillar caps; EnderCrystalRegistrar callback in World.h processes pending crystals on chunk load; dragon healing AI searches enderCrystalEntities_ within 32 blocks every 10 ticks, heals 1HP/10 ticks, stores healingCrystalId; crystal destruction breaks dragon healing link; player join sync sends S0E for all existing End crystals; nextEnderCrystalEntityId_ starting at 1400000)
281. **Lightning bolt entity** — ✅ Done (Java EntityLightningBolt parity: SpawnedLightning struct with EntityLightningBolt logic; spawnLightning creates entity + S2C SpawnGlobalEntity type 1 broadcast + ambient.weather.thunder/random.explode sounds; fire at impact gated by doFireTick gamerule + Difficulty::NORMAL/HARD check + 4 random nearby fires; tickLightning handles re-strike flashes, 3-block radius entity damage — players: 5.0f dealFireDamage + setFire(160), mobs: 5.0f damage; pig 90 → zombie pigman 57 with golden sword, creeper 50 → charged creeper via DW 17=1; weather system thunder integration spawns lightning at random player-nearby positions during thunderstorms)
282. **Minecart entity** — ✅ Done (Java EntityMinecart parity: SpawnedMinecart struct with EntityMinecart rail physics; spawnMinecart creates entity + S0E SpawnObject type 10 broadcast; tickMinecarts handles on-rail movement with rail type 66/27/28/157, powered rail acceleration, slope physics, rider steering via C0C SteerVehicle yaw/forward input, off-rail gravity + ground check; S18 EntityTeleport position sync; collision with solid blocks destroys minecart + dismounts rider via S1B AttachEntity; item 328 right-click on rail spawns minecart, survival consumption)
