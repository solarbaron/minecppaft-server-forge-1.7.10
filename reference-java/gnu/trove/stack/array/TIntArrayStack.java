...
Externalizable {
    ...
    private void reverse(int[] dest, int from, int to2) {
        if (from == to2) {
            return;
        }
        if (from > to2) {
    ...
    private void swap(int[] dest, int i2, int j2) {
        int tmp = dest[i2];
        dest[i2] = dest[j2];
        dest[j2] = tmp;
    }
    ...
    public boolean equals(Object o2) {
        ...
        TIntArrayStack that = (TIntArrayStack)o2;
    ...
    public void readExternal(ObjectInput in2) throws IOException, ClassNotFoundException {
        in2.readByte();
        this._list = (TIntArrayList)in2.readObject();
    }
}
