...
implements Cloneable {
    ...
    private ChannelFuture doBind(final SocketAddress localAddress) {
        ChannelFuture regPromise = this.initAndRegister();
        final Channel channel = regPromise.channel();
        final ChannelPromise promise = channel.newPromise();
        if (regPromise.isDone()) {
            AbstractBootstrap.doBind0(regPromise, channel, localAddress, promise);
        } else {
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
            ...
            return channel.newFailedFuture(t2);
        }
        ChannelPromise regPromise = channel.newPromise();
        this.group().register((Channel)channel, regPromise);
