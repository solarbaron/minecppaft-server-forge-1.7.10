...
extends PooledByteBuf<ByteBuffer> {
    ...
    public ByteBuf copy(int index, int length) {
        this.checkIndex(index, length);
        PooledUnsafeDirectByteBuf copy = (PooledUnsafeDirectByteBuf)this.alloc().directBuffer(length, this.maxCapacity());
        if (length != 0) {
    ...
    public ByteBuffer nioBuffer(int index, int length) {
        this.checkIndex(index, length);
        index = this.idx(index);
        return (ByteBuffer)((ByteBuffer)this.memory).duplicate().position(index).limit(index + length);
    }
