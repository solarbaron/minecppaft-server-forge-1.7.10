...
extends ByteToMessageDecoder {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in2) throws Exception {
        ...
        long frameLength = this.getFrameLength(in2, actualLengthFieldOffset);
        if (frameLength < 0L) {
    ...
    private long getFrameLength(ByteBuf in2, int actualLengthFieldOffset) {
        long frameLength;
        in2 = in2.order(this.byteOrder);
        switch (this.lengthFieldLength) {
            case 1: {
                frameLength = in2.getUnsignedByte(actualLengthFieldOffset);
                break;
            }
            case 2: {
                frameLength = in2.getUnsignedShort(actualLengthFieldOffset);
                break;
            }
            case 3: {
                frameLength = in2.getUnsignedMedium(actualLengthFieldOffset);
                break;
            }
            case 4: {
                frameLength = in2.getUnsignedInt(actualLengthFieldOffset);
                break;
            }
            case 8: {
                frameLength = in2.getLong(actualLengthFieldOffset);
                break;
            }
            default: {
