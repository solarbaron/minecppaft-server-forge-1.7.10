...
extends ZlibDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in2, List<Object> out) throws Exception {
        if (!in2.isReadable() && this.finished) {
            return;
        }
        if (this.crc != null) {
        ...
        try {
            ...
            while (!this.inflater.needsInput()) {
                ...
                int outputLength = this.inflater.inflate(outArray, outIndex = decompressed.arrayOffset() + decompressed.writerIndex(), length = outArray.length - outIndex);
                if (outputLength > 0) {
    ...
    private boolean readGZIPFooter(ByteBuf buf) {
        ...
        if (dataLength != readLength) {
            throw new CompressionException("Number of bytes missmatch. Expected: " + dataLength + ", Got: " + readLength);
        }
        return true;
    }
