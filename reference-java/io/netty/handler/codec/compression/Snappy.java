...
public class Snappy {
    ...
    public void encode(ByteBuf in2, ByteBuf out, int length) {
        ...
        out.writeByte(b2);
        int inIndex = in2.readerIndex();
        int baseIndex = in2.readerIndex();
        int maxIndex = length;
        ...
        if (maxIndex - inIndex >= 15) {
            ...
            block1: while (true) {
                ...
                do {
                    int base = inIndex;
                    int matched = 4 + Snappy.findMatchingLength(in2, candidate + 4, inIndex + 4, maxIndex);
        ...
        if (nextEmit < maxIndex) {
            Snappy.encodeLiteral(in2, out, maxIndex - nextEmit);
        }
    }
