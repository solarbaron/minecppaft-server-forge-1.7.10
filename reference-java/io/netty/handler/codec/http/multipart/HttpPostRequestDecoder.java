...
public class HttpPostRequestDecoder {
    ...
    private void parseBodyAttributesStandard() throws ErrorDataDecoderException {
        ...
        try {
                ...
                switch (this.currentStatus) {
                    ...
                    case FIELD: {
                        if (read == '&') {
                            this.currentStatus = MultiPartStatus.DISPOSITION;
                            ampersandpos = currentpos - 1;
                            this.setFinalBuffer(this.undecodedChunk.slice(firstpos, ampersandpos - firstpos).retain());
                            firstpos = currentpos;
                            contRead = true;
                            continue block7;
                        }
            ...
            if (this.currentStatus == MultiPartStatus.FIELD) {
                this.currentAttribute.addContent(this.undecodedChunk.slice(firstpos, currentpos - firstpos).retain(), false);
                firstpos = currentpos;
            }
    ...
    private static String decodeAttribute(String s2, Charset charset) throws ErrorDataDecoderException {
        ...
        try {
            return URLDecoder.decode(s2, charset.name());
        }
        catch (UnsupportedEncodingException e2) {
            throw new ErrorDataDecoderException(charset.toString(), e2);
        }
        catch (IllegalArgumentException e3) {
            throw new ErrorDataDecoderException("Bad string: '" + s2 + '\'', e3);
        }
    }
    ...
    private void readFileUploadByteMultipartStandard(String delimiter) throws NotEnoughDataDecoderException, ErrorDataDecoderException {
        ...
        ByteBuf buffer = this.undecodedChunk.slice(readerIndex, lastPosition - readerIndex).retain();
        if (found) {
            try {
    ...
    private void loadFieldMultipartStandard(String delimiter) throws NotEnoughDataDecoderException, ErrorDataDecoderException {
        ...
        try {
            ...
            if (found) {
                try {
                    this.currentAttribute.addContent(this.undecodedChunk.slice(readerIndex, lastPosition - readerIndex).retain(), true);
                }
                catch (IOException e2) {
                    throw new ErrorDataDecoderException(e2);
                }
            ...
            try {
                this.currentAttribute.addContent(this.undecodedChunk.slice(readerIndex, lastPosition - readerIndex).retain(), false);
            }
            catch (IOException e3) {
                throw new ErrorDataDecoderException(e3);
            }
