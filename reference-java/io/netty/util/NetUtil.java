...
public final class NetUtil {
        ...
        block9: for (int i2 = 0; i2 < length; ++i2) {
            ...
            switch (c2) {
                ...
                case '%': {
                    ...
                    try {
                        Integer.parseInt(ipAddress.substring(i2 + 1));
                        continue block9;
                    }
    ...
    static {
        ...
        if (LOOPBACK_IF != null) {
            ...
            Enumeration<InetAddress> addrs = LOOPBACK_IF.getInetAddresses();
            while (addrs.hasMoreElements()) {
                InetAddress a2 = addrs.nextElement();
                if (localhost == null) {
                    logger.debug("Loopback address: {} (primary)", (Object)a2);
                    localhost = a2;
                    continue;
                }
                logger.debug("Loopback address: {}", (Object)a2);
            }
        }
        ...
        try {
            in2 = new BufferedReader(new FileReader("/proc/sys/net/core/somaxconn"));
            somaxconn = Integer.parseInt(in2.readLine());
            logger.debug("/proc/sys/net/core/somaxconn: {}", (Object)somaxconn);
        }
        catch (Exception e6) {
        }
        finally {
